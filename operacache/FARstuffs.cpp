...
{
	fInfo.Message(fInfo.ModuleNumber,FMSG_LEFTALIGN, NULL, msg, msgsize, 0);
}
...
void ConvertNameToReal(CString &name ) 
{
	...
	//return name;
	//CString ret;
	//FSF.ConvertNameToReal(name, ret.GetBuffer(1000), 1000);
	//ret.ReleaseBuffer();
	//return ret;
}
...
{
	PluginPanelItem *ffd = GetFarPluginPanelItem(another?PANEL_PASSIVE:PANEL_ACTIVE, selected?FCTL_GETSELECTEDPANELITEM:FCTL_GETPANELITEM, i);
	if (ffd)
	{
