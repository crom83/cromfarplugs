...
long PackCheckGTA3SFX(const char * FileName, PUCHAR BaseString, long readed, long &HeadOffset, long &SomeOffset, long &FileSize, long &FileOffset, struct RIP_FILE_INFO **finfo, long &FilesNumber, long WorkFileSize, struct RIP_SERVICE_FUNC *func, struct RIP_FLAGS *flags)
{       
#pragma pack(1)
    struct FATInfo{
        long Offset;
...
#pragma pack(8)
    ...
    struct _stat buf;
    _stat(base, &buf);
    if (buf.st_size%3==0) GTA1 = true;    
    
    if (GTA1)
         nFiles = buf.st_size/12;
    else nFiles = buf.st_size/20;
    ...
    *finfo = fi;    
    ...
    for (int i = 0; i<FilesNumber; i++){    
        ...
        fi[i].UnpSize     = MAKELONG(FATItem.Bitrate, GTA1);
        ...
        sprintf(s2, "Freq: %d", FATItem.Bitrate);
...
BOOL GetFileGTA3SFX(const char * PackName, const char * FileName, const char * PathName, long StartOffset, long FileSize, long UnpSize, struct RIP_SERVICE_FUNC* func, struct RIP_FLAGS *flags)
{
    ...
    if (!GTA1) l = Bitrate*2;//SampleRate*AverageByte
    else       l = Bitrate;
    func->AppendToFile(dstpath, &l, 4);
